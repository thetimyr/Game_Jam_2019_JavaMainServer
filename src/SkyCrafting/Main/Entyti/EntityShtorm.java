...
public class EntityShtorm extends EntityMonster
{
    Spawner spawner;
	...
	public EntityShtorm(final Spawner spawner) {
        ...
        this.setCustomName(ChatColor.translateAlternateColorCodes('&', "&f&lШтурмовик " + (int)this.getHealth() + " ❤"));
    ...
    public void die() {
        ...
        if (this.killer != null) {
            ...
            for (final String key : this.attackers.keySet()) {
                ...
                q = money;
                Bukkit.getPlayer(key).sendMessage(ChatColor.GREEN + "На ваш счет было зачисленно " + "2 убийств,8 опыта и " + ChatColor.GREEN + money + "$");
                Levels.kills.put(this.killer.getName(), Levels.kills.get(this.killer.getName()) + 2);
                Levels.xp.put(this.killer.getName(), Levels.xp.get(this.killer.getName()) + 8);
 
            }
            Bosslistener.HoloDiad((org.bukkit.entity.Entity)this.bukkitEntity, q, 2, 8);
            Bukkit.broadcastMessage((String)ChatColor.translateAlternateColorCodes('&', "&f&lШтурмовик &cбыл повержен, нападающие получают по " + q + "$!"));
        }
        super.die();
    }
}
