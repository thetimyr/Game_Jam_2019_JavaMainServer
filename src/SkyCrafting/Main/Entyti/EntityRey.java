...
public class EntityRey extends EntityMonster
{
    Spawner spawner;
	...
	public EntityRey(final Spawner spawner) {
        ...
        this.getAttributeInstance(GenericAttributes.maxHealth).setValue(400.0);
        ...
        this.setEquipment(2, new net.minecraft.server.v1_8_R3.ItemStack((Item)Items.LEATHER_CHESTPLATE));
        this.setEquipment(3, new net.minecraft.server.v1_8_R3.ItemStack((Item)Items.LEATHER_LEGGINGS));
        this.setEquipment(1, new net.minecraft.server.v1_8_R3.ItemStack((Item)Items.LEATHER_BOOTS));
        this.setEquipment(0, new net.minecraft.server.v1_8_R3.ItemStack(Items.WOODEN_SWORD));
        this.setCustomName(ChatColor.RED + "Рей " + (int)this.getHealth() + "❤ ");
    ...
    public void die() {
        ...
        if (this.killer != null) {
            ...
            drop.add(new org.bukkit.inventory.ItemStack(Material.IRON_INGOT,30));
            ...
            drop.add(new org.bukkit.inventory.ItemStack(Material.SEEDS));
            drop.getlist().addAll(Bosslistener.List(0));
            drop.dropedBoss();
            int q = 0;
            for (final String key : this.attackers.keySet()) {
                ...
                q = money;
                Bukkit.getPlayer(key).sendMessage(ChatColor.GREEN + "На ваш счет было зачисленно " + "7 убийств, 7 Опыта и " + ChatColor.GREEN + money + "$");
                Levels.kills.put(this.killer.getName(), Levels.kills.get(this.killer.getName()) + 7);
    			...
    			if(Levels.getCrystal(Bukkit.getPlayer(key)) == 5) {
        			Levels.crystal.put(Bukkit.getPlayer(key).getName(), Levels.crystal.get(Bukkit.getPlayer(key).getName()) + 1);
        			Main.econ.depositPlayer(Bukkit.getPlayer(key), (double)10000);
        			Bukkit.getPlayer(key).getInventory().addItem(new ItemStack(Material.GOLD_INGOT,3));
        			Bukkit.getPlayer(key).sendMessage("§aВы успешно выполнили 6 квест!");
        			JediScoreBoard.UpdateList(Bukkit.getPlayer(key));
        			JediScoreBoard.updateScoreboard(Bukkit.getPlayer(key));
        			
    			}
            }
            Bosslistener.HoloDiad((org.bukkit.entity.Entity)this.bukkitEntity, q, 7, 7);
            Bukkit.broadcastMessage((String)ChatColor.translateAlternateColorCodes('&', "&4Рей &cбыл повержен, нападающие получают по " + q + "$!"));
        }
        super.die();
    }
}