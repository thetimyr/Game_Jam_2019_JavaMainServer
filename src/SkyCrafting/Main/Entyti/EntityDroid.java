...
public class EntityDroid extends EntityMonster
{
    Spawner spawner;
	...
	public EntityDroid(final Spawner spawner) {
        ...
        this.setCustomName(ChatColor.translateAlternateColorCodes('&', "&4Боевой дроид " + (int)this.getHealth() + " ❤"));
    ...
    public void die() {
        ...
        if (this.killer != null) {
            ...
            drop.add(new ItemStack(Material.IRON_INGOT, 5-10));
            ...
            for (final String key : this.attackers.keySet()) {
                ...
                q = money;
                Bukkit.getPlayer(key).sendMessage(ChatColor.GREEN + "На ваш счет было зачисленно " + "3 убийств и 5 опыта и " + ChatColor.GREEN + money + "$");
                Levels.kills.put(this.killer.getName(), Levels.kills.get(this.killer.getName()) + 3);
                Levels.xp.put(this.killer.getName(), Levels.xp.get(this.killer.getName()) + 5);
            ...
            Bosslistener.HoloDiad((org.bukkit.entity.Entity)this.bukkitEntity, q, 3, 5);
            Bukkit.broadcastMessage((String)ChatColor.translateAlternateColorCodes('&', "&4Боевой дроид&4&l! &cбыл повержен, нападающие получают по " + q + "$!"));
        }
        super.die();
    }
}