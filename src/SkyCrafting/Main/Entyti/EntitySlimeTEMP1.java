...
public class EntitySlimeTEMP1 extends EntityMonster
{
    int totalDamage;
    int hp_delay;
    int debuff;
	...
	public EntitySlimeTEMP1(final World world) {
        ...
        this.setCustomName(ChatColor.RED + "Мини-Миньйон " + (int)this.getHealth() + "❤ ");
    ...
    public void die() {
        if (this.killer instanceof EntityPlayer) {
            ...
            Main.econ.depositPlayer((OfflinePlayer)p,5.0);
            p.sendMessage(ChatColor.GREEN + "На ваш счет было зачисленно 5$, за убийство миньйона");
        }
        super.die();
    }
    ...
    public void debuff() {
        new BukkitRunnable() {
            public void run() {
                for (final org.bukkit.entity.Entity entity : EntitySlimeTEMP1.this.getBukkitEntity().getNearbyEntities(7.0, 5.0, 7.0)) {
