...
public class EntityMagma extends EntityMonster
{
    Spawner spawner;
    int delay;
    int hp_delay;
	...
	public EntityMagma(final Spawner spawner) {
        ...
        this.getAttributeInstance(GenericAttributes.ATTACK_DAMAGE).setValue(100.0);
        ...
        this.setCustomName(ChatColor.RED + "Лавовый куб " + (int)this.getHealth() + "❤ ");
    ...
    public void die() {
        ...
        if (this.killer != null) {
            ...
            drop.add(new ItemStack(new ItemStack(Material.getMaterial(351), 1, (short)15)));
            ...
            for (final String key : this.attackers.keySet()) {
                final int money = 10600 / this.attackers.size();
                Main.econ.depositPlayer(key, (double)money);
                if (Bukkit.getPlayer(key) == null) {
                    continue;
                }
                q = money;
                Bukkit.getPlayer(key).sendMessage(ChatColor.GREEN + "На ваш счет было зачисленно " + "10 убийств и 17 опыта и " + ChatColor.GREEN + money + "$");
                Levels.kills.put(this.killer.getName(), Levels.kills.get(this.killer.getName()) + 10);
                Levels.xp.put(this.killer.getName(), Levels.xp.get(this.killer.getName()) + 17);
            ...
            Bosslistener.HoloDiad((org.bukkit.entity.Entity)this.bukkitEntity, q, 10, 17);
            Bukkit.broadcastMessage((String)ChatColor.translateAlternateColorCodes('&', "&4Лавовый куб &cбыл повержен, нападающие получают по " + q + "$!"));
        }
        super.die();
    }
}
