...
public class EntityPavuk extends EntityMonster
{
    Spawner spawner;
    int delay;
    int hp_delay;
	...
	public EntityPavuk(final Spawner spawner) {
        ...
        this.getAttributeInstance(GenericAttributes.maxHealth).setValue(65.0);
        ...
        this.setCustomName(ChatColor.RED + "Пещерный шнюк " + (int)this.getHealth() + "❤ ");
    ...
    public void die() {
        ...
        this.droped(0.09, Material.IRON_LEGGINGS);
        this.droped(0.05, Material.IRON_BOOTS);
        this.droped(0.08, Material.IRON_HELMET);
        ...
        if (this.killer instanceof EntityPlayer) {
            ...
            Main.econ.depositPlayer((OfflinePlayer)p, 60);
            Levels.xp.put(this.killer.getName(), Levels.xp.get(this.killer.getName()) + 1);
            p.sendMessage(ChatColor.GREEN + "На ваш счет было зачисленно 60$ , 2 опыт");
            Bosslistener.HoloDiad((org.bukkit.entity.Entity)this.bukkitEntity, 60, 0, 2);
