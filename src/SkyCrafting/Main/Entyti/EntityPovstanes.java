...
public class EntityPovstanes extends EntityMonster
{
    Spawner spawner;
	...
	public EntityPovstanes(final Spawner spawner) {
        ...
        this.getAttributeInstance(GenericAttributes.maxHealth).setValue(900.0);
        ...
        this.getAttributeInstance(GenericAttributes.ATTACK_DAMAGE).setValue(80.0);
        ...
        this.setCustomName(ChatColor.translateAlternateColorCodes('&', "&a&lУсиленный повстанец " + (int)this.getHealth() + " ❤"));
    ...
    public void die() {
        ...
        if (this.killer != null) {
            ...
            drop.add(new ItemStack(Material.RECORD_4));
            ...
            for (final String key : this.attackers.keySet()) {
                final int money = 10000 / this.attackers.size();
                Main.econ.depositPlayer(key, (double)money);
                if (Bukkit.getPlayer(key) == null) {
                    continue;
                }
                q = money;
                Bukkit.getPlayer(key).sendMessage(ChatColor.GREEN + "На ваш счет было зачисленно " + "15 убийств,25 опыта и " + ChatColor.GREEN + money + "$");
            ...
            Bosslistener.HoloDiad((org.bukkit.entity.Entity)this.bukkitEntity, q, 15, 25);
            Bukkit.broadcastMessage((String)ChatColor.translateAlternateColorCodes('&', "&a&lУсиленный повстанец &cбыл повержен, нападающие получают по " + q + "$!"));
        }
        super.die();
    }
}